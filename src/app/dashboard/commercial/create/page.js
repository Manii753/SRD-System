...
export default function CreateSRDPage() {
  ...
  useEffect(() => {
    ...
    // fetch dynamic fields for commercial department
    (async function fetchDynamic() {
      try {
        const res = await fetch('/api/newField?department=commercial');
      ...
      const response = await fetch('/api/srd', {
        ...
        body: JSON.stringify({
          ...
          dynamicFields: dynamicDefs.map(d => ({
            field: d._id,
            department: d.department || 'commercial',
            name: d.name,
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {dynamicDefs.length === 0 ? (
                <div className="text-gray-600">No dynamic fields defined for Commercial. Add fields from Admin → Manage SRD Fields.</div>
              ) : (
